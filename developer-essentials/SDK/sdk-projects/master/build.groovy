...
void executeCommand(String[] args) {
    ...
    cli.with {
        ...
        th longOpt: 'toolkithelp', 'Shows the standard toolkit help message exits'
        ...
        pr longOpt: 'project', args: 1, argName: 'project', 'Specifies to project to run the command against'
...
${printTask('da-subset-filesystem-example', 'Demonstrates a data access on demand sub set')}
${printTask('da-subset-documents-example', 'Demonstrates a data access on demand sub set - including linked documents')}
...
${printTask('packXap', 'Compiles the assembies in to the Apollo.xap')}
...
Examples of use:
build -pr da-arload-filesystem-example -t generateMappingJar -x C:\\IBM\\iap\\SDK\\sdk-projects\\master\\build\\IAP-Deployment-Toolkit\\configuration\\examples\\schemas\\en_US\\law-enforcement-schema.xml -o C:\\IBM\\iap\\SDK\\sdk-projects\\da-arload-filesystem-example\\schema-mapping-jar\\schema.jar
...
"""
    def help =
"""This script configures the Intelligence Analysis Platform's Developer Essentials.
$usage"""
...
String printTask(String taskName, String description) {
    ...
    description = wordWrap(description, paddedTaskName, 38, 40)
    return description
}

def wordWrap(text, firstLine, colWidth, indent = 0) {
...
String getPaddedLongOption(CliBuilder cli, String option) {
    ...
    def rightPad = 35
    return padRight(longOption + optionalArgName, 35)
}
...
void executeToolkit(OptionAccessor options, ArrayList<String> args) {
    ...
    command += ["setup"]
    command += args
    File scriptsDir = new File("$baseScriptsDir/../../IAP-Deployment-Toolkit")
...
baseScriptsDir = "$currentLocationDir/build/IAP-Deployment-Toolkit/scripts"
gradlePath = normalisedPath("$currentLocationDir/../../../IAP-Deployment-Toolkit/tools/gradle/bin")
executeCommand(args)
