...
When you build the audit logging to file example, you create a JAR file that contains an implementation of the `IAuditLogger` interface. To enable audit functionality, you must add the JAR file to an existing i2 Analyze deployment, and then configure the platform to use the class that the JAR file contains.
