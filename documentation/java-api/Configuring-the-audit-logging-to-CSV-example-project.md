...
6.  Run the following command to start the i2 Analyze server:
    ...
    The supplied version of `CSVAudit.properties` specifies the `wlp\usr\servers\opal-server\logs\opal-services` directory as the target for audit CSV files.  If you are using analyze-deployment-tooling for your deployment of i2 Analyze, update the path to `/logs`. The files are named `auditn.csv`, where n is an integer that increments with each new file.
