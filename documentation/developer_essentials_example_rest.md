...
14. Edit the `server.xml` for your WebSphere Application Server Liberty server to exclude the Apache Wink JAR files, and to use the `jaxrs-1.1` feature:
    a.  Navigate to the `C:\IBM\i2analyze\deploy-dev\wlp\usr\servers\i2analyze` directory, and then open the `server.xml` file in an XML editor.
    b.  In `server.xml`, locate the `<webApplication>` element whose `id` attribute has the value `da-subset-rest-example`.
    c.  The web application contains a child `<classLoader>` element. Within `<classLoader>` is a `<fileset>` element. Add an `excludes` attribute to the `<fileset>` element that matches the following example:
    ...
    d.  In the same file, add the following child `<feature>` element to the `<featureManager>` element to add the `jaxrs-1.1` feature to the server:
...
4.  Ensure that the JAR file from `da-example-common` is loaded correctly into the deployment assembly for the `da-subset-rest-example` project:
    a.  In Package Explorer, right-click **da-subset-rest-example**, and select **Properties** to display the Properties window.
