...
3.  Open the `http-custom-rewrite-rules.txt` file. By default, the file is in the `SDK\sdk-projects\master\build\toolkit\configuration\environment\proxy` directory.
    1.  Add the following code between the `!Start_After_Rules!` and `!End_After_Rules!` lines:
...
4.  To redeploy i2 Analyze, run the following command:
